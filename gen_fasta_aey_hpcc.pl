...
foreach my $key (keys %col0_chrom) {
	if ($col0_chrom{$key} =~ m/ChrM/ || $col0_chrom{$key} =~ m/ChrC/) {
		next;
	}
	else {
		if ($col0_up{$key} >= $col0_down{$key}) {
			open (SAMTOOLSA,"/opt/software/SAMTools/1.2--GCC-4.4.5/bin/samtools faidx /mnt/home/yoccaala/04_Edger/athal_cns/$opt_a $col0_chrom{$key}:$col0_down{$key}-$col0_up{$key} |");
			while (my $samtool = <SAMTOOLSA>) {
				#my $info = $a_id;
				#$samtool =~ s/Chr$a_chrom/$info/;
				$samtool =~ s/$col0_chrom{$key}/$col0_chrom{$key}_$key/;
				$samtool =~ s/:/_/;
				$samtool =~ s/-/_/;
		...
		if ($col0_down{$key} >= $col0_up{$key}) {
			open (SAMTOOLSA,"/opt/software/SAMTools/1.2--GCC-4.4.5/bin/samtools faidx /mnt/home/yoccaala/04_Edger/athal_cns/$opt_a $col0_chrom{$key}:$col0_up{$key}-$col0_down{$key} |");
			while (my $samtool = <SAMTOOLSA>) {
				#my $info = $a_id;
				#$samtool =~ s/Chr$a_chrom/$info/;
				$samtool =~ s/$col0_chrom{$key}/$col0_chrom{$key}_$key/;
...
foreach my $key (keys %acc_chrom) {
	if ($acc_up{$key} >= $acc_down{$key}) {
		open (SAMTOOLSA,"/opt/software/SAMTools/1.2--GCC-4.4.5/bin/samtools faidx /mnt/home/yoccaala/04_Edger/athal_cns/$opt_a $acc_chrom{$key}:$acc_down{$key}-$acc_up{$key} |");
		while (my $samtool = <SAMTOOLSA>) {
			#my $info = $a_id;
			#$samtool =~ s/Chr$a_chrom/$info/;
			$samtool =~ s/$acc_chrom{$key}/$acc_chrom{$key}_$key/;
			$samtool =~ s/:/_/;
			$samtool =~ s/-/_/;
	...
	if ($acc_down{$key} >= $acc_up{$key}) {
		open (SAMTOOLSA,"/opt/software/SAMTools/1.2--GCC-4.4.5/bin/samtools faidx /mnt/home/yoccaala/04_Edger/athal_cns/$opt_a $acc_chrom{$key}:$acc_up{$key}-$acc_down{$key} |");
		while (my $samtool = <SAMTOOLSA>) {
			#my $info = $a_id;
			#$samtool =~ s/Chr$a_chrom/$info/;
			$samtool =~ s/$acc_chrom{$key}/$acc_chrom{$key}_$key/;
